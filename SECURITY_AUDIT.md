...
- **Resolved in the image pass:** SEC-07 (reviewed semver-compatible transitive floors; built-image production audit now reports zero advisories) and SEC-15's current build/deployment risks (approved dsh commit, pinned Node 24/Trixie base digest, reproducible security patch, fail-closed checks, and content-addressed production deployment). All tenant containers were recreated on the audited image while preserving their two data volumes.
...
- **Resolved in the egress pass:** a tenant egress firewall (`firewall/tenant-egress.nft`, applied by `run-portal.sh`) blocks all RFC1918 private ranges (192.168/16, 10/8, 172.16/12) while preserving the WSL2 gateway/DNS and public internet egress. A live LAN scan confirmed tenant containers cannot reach the router, mesh nodes, smart TV, or other LAN devices, while public internet and model-provider APIs remain reachable.
...
- **Resolved in the lifecycle pass:** the Node 24/Trixie migration — the image base moved from `node:22-bookworm-slim` to `node:24-trixie-slim` (Node 24 LTS, Debian 13), rebuilt, audit-clean, and rolled out to all tenants with volumes preserved.
- **Still open:** named-volume quota/periodic orphan-reconciliation portions of SEC-11/SEC-12, DEP-02, service-credential rotation at the external provider, broader checked-in security tests/CI, and the P3 registrable-domain redesign.
...
**Status (2026-08-16): substantially remediated.** Production now requires rootless Podman and `pasta` preflight; every tenant has explicit CPU/memory/swap/PID limits, bounded logs/tmpfs, a read-only root, all capabilities dropped, no-new-privileges, and loopback-only publication. A tenant egress firewall (`firewall/tenant-egress.nft`, applied by `run-portal.sh`) blocks all RFC1918 private ranges (192.168/16, 10/8, 172.16/12) while preserving the WSL2 gateway/DNS and public internet egress. Live probes confirmed tenant containers cannot reach sibling containers, the portal, the host, cloud metadata, or any LAN device, while public internet and model-provider APIs remain reachable. Named-volume quotas remain unresolved on this storage backend.
...
**Status (2026-08-16): remediated.** The source commit and base digest are pinned, builds use a clean archive and exact context allowlist, source/security patches and the zero-advisory production audit fail closed, and production accepts only a content-addressed `sha256:` image ID. The base is now `node:24-trixie-slim` (Node 24 LTS, Debian 13), giving the longest combined Node + OS support window of the glibc Node images.
